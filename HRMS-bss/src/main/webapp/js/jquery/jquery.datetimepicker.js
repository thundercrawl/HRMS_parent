				...
				calendar
					.on('click.xdsoft','td',function() {
						...
						if( $this.hasClass('xdsoft_disabled') ){
							...
							//var currentDate = "2016-07-28";
							var currentDateYear = currentDate.substring(0,4);
							var cuttentDateMonth = currentDate.substring(5,7);
							var cuttentDateDay = currentDate.substring(8);
							currentTime.setFullYear( $this.data('year') );
							currentTime.setMonth( $this.data('month') );
							currentTime.setDate( $this.data('date') );
							if(cuttentDateDay<25){
								var beforeDateMonth = Number(cuttentDateMonth)-1;
								if((beforeDateMonth+"").length==1){
									beforeDateMonth = "0"+beforeDateMonth;
								}
								var a = new Date(currentDateYear+"-"+beforeDateMonth+"-25").Format("yyyy-MM-dd");
								var b = new Date(currentTime.getTime()).Format("yyyy-MM-dd");
								/*if(b<=a)//大于上个月的25号
									return false;*/
							}
							if(cuttentDateDay>25){
								var a = new Date(currentDateYear+"-"+cuttentDateMonth+"-25").Format("yyyy-MM-dd");
								var b = new Date(currentTime.getTime()).Format("yyyy-MM-dd");
								/*if(b<=a)//大于上个月的25号
									return false;*/
							}
						...
						if(input.attr('id')=="endDate"){
							endDateVal = input.val();
							var endDateYear = endDateVal.substring(0,4);
							var endDateMonth = endDateVal.substring(5,7);
							var endDateDate = endDateVal.substring(8);
							if(Number(input.val()!=""&&_xdsoft_datetime.str().substring(0,4))==Number(endDateYear)&&Number(_xdsoft_datetime.str().substring(5,7))==Number(endDateMonth))
								input.val( _xdsoft_datetime.str() );
						}else{
							input.val( _xdsoft_datetime.str() );
						}
