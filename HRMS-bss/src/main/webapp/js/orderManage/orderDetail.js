$(function() {
	...
	$('.deleteParam').on('click', function() {
		$('#page-body').load("filter/orderManage/deleteOfcOrderDetail?orderDetailId="+$(this).attr('data-orderDetailId'));
	})
	$('.deleteAsset').on('click', function() {
		var orderId = $('#orderId').val();
		$('#content').load("filter/orderManage/deleteAsset?assetId="+$(this).attr('data-assetId')+"&assetTypeStr="+$(this).attr('data-assetTypeStr')+"&orderId="+orderId);
	});
	...
	$('.detail').on('click', function() {
		$('#page-body').load("filter/orderManage/ofcOrderDetailPage?orderId="+$(this).attr('data-orderId'));
	});
	$('#returnBtn').on('click', function() {//订单明细返回按钮
		...
		$('#content').load("filter/orderManage/returnOfcOrderSearch?companyName="+ companyName+"&status="+status+"&userName="+userName+"&orderStartTime="+orderStartTime+"&orderEndTime="+orderEndTime+"&page=0");
	})
...
function add() {
	if (window.confirm("确定提交？")) {
		...
		$.ajax({
			url : 'filter/orderManage/addOfcOrderDetail',
			type : 'post',
			data : {
				'orderId' : orderId,
...
function update() {
	if (window.confirm("确定提交？")) {
		...
		var status = $("#status").val();
		$.ajax({
			url : 'filter/orderManage/updateOfcOrderDetail',
			type : 'post',
			data : {
				'orderId' : orderId,
				'orderDetailId' : orderDetailId,
