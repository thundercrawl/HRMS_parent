	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "filter/project/addprojectPic",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div class="img-group"><ul><li><img src="img/bg.jpg"/><span class="file_del" data-index="0" title="删除"></span></li></ul></div>'
				    ...
				    html += '							<img class="upload_image" src="img/upimg/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "img/upimg/fileType/";
